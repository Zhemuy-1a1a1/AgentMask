# AgentMask · 单文件可导入 Agent 包

<p align="center">
  <strong>One Markdown. One Soul.</strong><br />
  <strong>戴上 AgentMask，化身专属专家。</strong><br />
  <em>Importable one-file agent packs.</em>
</p>

<p align="center">
  <a href="./LICENSE"><img alt="License" src="https://img.shields.io/badge/license-CC%20BY--NC%204.0-orange" /></a>
  <a href="./agents/INDEX.md"><img alt="Packs" src="https://img.shields.io/badge/packs-28-blue" /></a>
  <a href="./README.zh-CN.md"><img alt="Chinese" src="https://img.shields.io/badge/README-中文-red" /></a>
  <a href="./README.en.md"><img alt="English" src="https://img.shields.io/badge/README-English-green" /></a>
</p>

<p align="center">
  <a href="./README.en.md">English Guide</a> ·
  <a href="./README.zh-CN.md">中文说明</a> ·
  <a href="./QUICKSTART.en.md">Quick Start EN</a> ·
  <a href="./QUICKSTART.zh-CN.md">快速开始 CN</a> ·
  <a href="./agents/INDEX.md">Agent Index / Agent 列表</a> ·
  <a href="./UPGRADE.md">Upgrade / 升级路径</a> ·
  <a href="./PUBLIC_RELEASE_SCOPE.md">Public Scope / 公开边界</a> ·
  <a href="./EDITIONS.md">Edition Guide / 版本线</a>
</p>

---

## What this repo is / 这个仓库是干什么的

This repo gives you **standard public one-file agent packs** for real work.
Open a markdown file, paste it into your AI agent instructions, and start using it.

这个仓库提供一批面向真实场景的 **标准公开版单文件 Agent 包**。
打开一个 markdown 文件，把它贴进你的 AI Agent 指令区，就能开始使用。

---

## Good fit if you want to / 如果你想做这些事，这个仓库适合你
- create content faster / 更快做内容
- reply to customers better / 更稳地做客服回复
- structure sales follow-up / 把跟单流程理顺
- build daily-life helpers / 做旅行、健身、打卡、英语陪练等生活型助手
- turn prompts into products / 把 prompt 整理成交付物
- test AI monetization ideas fast / 更快验证 AI 变现方向

---

## 3 steps to use / 3 步上手

### 1. Pick one pack / 选一个包
Go to [agents/INDEX.md](./agents/INDEX.md) and choose the scenario closest to your task.
去 [agents/INDEX.md](./agents/INDEX.md) 里挑一个最接近你需求的场景。

### 2. Copy the file / 复制文件
Open the `.md` file and copy the whole content.
打开对应 `.md` 文件，整份复制。

### 3. Paste into your AI agent / 粘贴到你的 Agent
Paste it into your system prompt / instructions / custom agent setup.
把内容粘贴到系统提示词 / Instructions / 自定义 Agent 配置里。

If you are new, read:
如果你是第一次用，先看：
- [Quick Start EN](./QUICKSTART.en.md)
- [快速开始 CN](./QUICKSTART.zh-CN.md)

---

## Start with these 6 / 推荐先看这 6 个

1. [小红书运营助手](./agents/zh-CN/小红书运营助手.md) / [xiaohongshu-operator.md](./agents/en/xiaohongshu-operator.md)
2. [旅行规划Agent](./agents/zh-CN/旅行规划Agent.md) / [travel-planner-agent.md](./agents/en/travel-planner-agent.md)
3. [健康健身Agent](./agents/zh-CN/健康健身Agent.md) / [fitness-coach.md](./agents/en/fitness-coach.md)
4. [客服回复助手](./agents/zh-CN/客服回复助手.md) / [customer-support-assistant.md](./agents/en/customer-support-assistant.md)
5. [项目管理Agent](./agents/zh-CN/项目管理Agent.md) / [project-manager-agent.md](./agents/en/project-manager-agent.md)
6. [英语口语陪练Agent](./agents/zh-CN/英语口语陪练Agent.md) / [english-speaking-coach.md](./agents/en/english-speaking-coach.md)

---

## Coverage / 当前覆盖面

### Business / 商务向
content, customer support, sales follow-up, quotations, coding, analysis, meeting notes, project management, weekly reports, reconciliation, hiring screening, ecommerce operations, social media management, sales ledger

### Lifestyle / 生活向
fitness, travel planning, habit tracking, English speaking practice, reading notes

### Study & Utility / 学习与工具向
AI image prompting, daily updates, email scheduling, smart home routines, academic research, knowledge-base Q&A, reading companion

For the full list, see:
完整列表见：
[agents/INDEX.md](./agents/INDEX.md)

---

## Public standard edition vs upgraded versions / 公开标准版与升级版的区别

This repo is the **public standard edition**.
It is complete enough to copy, import, and use directly.

If you want stronger production usability, lower API cost, better China-friendly setup, or custom adaptation, the upgrade path is now:
- **Optimized version**
- **Advanced (custom) version**

Even after upgrading, the core product direction stays the same:
**one agent = one core `.md` file**.

这个仓库放的是**公开标准版**。
它已经足够用于直接复制、导入、开始使用。

如果你想要更强的生产可用性、更低 API 成本、更友好的国内源适配，或者按业务深改，当前升级路径统一为：
- **增强优化版**
- **进阶（定制）版**

无论升级到哪一层，核心产品方向都不变：
**一个 Agent = 一个核心 md 文件。**

See: [UPGRADE.md](./UPGRADE.md)

---

## Public release note / 公开说明
- The public repo now focuses on the **standard public edition**, not just a trial layer.
- Some newly added packs are currently available first in `agents/zh-CN/` and will get English aliases later.
- More optimized low-cost / China-source / custom versions are intentionally kept outside the public repo.

- 当前公开仓库以 **标准公开版** 为主，不再只是试吃层。
- 部分新增 Agent 先进入 `agents/zh-CN/` 公开目录，英文别名后续再补。
- 更强的低成本版 / 国内源版 / 定制版，会有意保留在公开仓之外。

## Useful links / 常用入口
- [README.en.md](./README.en.md)
- [README.zh-CN.md](./README.zh-CN.md)
- [QUICKSTART.en.md](./QUICKSTART.en.md)
- [QUICKSTART.zh-CN.md](./QUICKSTART.zh-CN.md)
- [examples/sample-inputs.md](./examples/sample-inputs.md)
- [examples/sample-outputs.md](./examples/sample-outputs.md)
- [agents/INDEX.md](./agents/INDEX.md)
- [PUBLIC_RELEASE_SCOPE.md](./PUBLIC_RELEASE_SCOPE.md)

---

## License / 许可证

[CC BY-NC 4.0](./LICENSE)

You can learn from it, adapt it, and share it with attribution.
You may **not** directly resell the public version for commercial use.

你可以学习、改写、传播（需署名），
但**不能**直接把公开版拿去商用倒卖。
